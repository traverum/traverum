...
Suppliers managing their daily booking workflow. Most will interact via email one-click actions; the dashboard provides the full picture.

## Key stories

- Supplier gets email "New booking request" → clicks Accept → guest gets payment link → done (never opened the dashboard)
- Supplier opens dashboard → sees 3 pending requests sorted by urgency → accepts two, declines one with alternative suggestion
- Supplier checks upcoming tab → sees tomorrow's bookings with guest contact info → prepares for the day
- Guest hasn't paid after approval → supplier sees it in "Awaiting Payment" tab → knows to wait
