...
interface HostPageProps {
  params: Promise<{ hotelSlug: string; hostSlug: string }>
}
...
export default async function HostHotelPage({ params }: HostPageProps) {
  const { hotelSlug, hostSlug } = await params

  const hotel = await getHotelBySlug(hotelSlug)
  if (!hotel) notFound()

  const host = await getHostBySlug(hostSlug, hotel.id)
  if (!host) notFound()
    ...
    <div className="px-4 py-6">
      <div className="container space-y-10">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="text-3xl sm:text-4xl font-heading font-light text-heading-foreground">
            {host.display_name}
          </h1>
